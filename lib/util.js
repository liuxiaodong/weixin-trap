...
/**
  ...
  *
  */
exports.saveOauthToken = function(appid, token, callback){
  if(!this.oauthTokenStore) this.oauthTokenStore = {};
  this.oauthTokenStore[appid] = {
    accessToken: token.accessToken,
  ...
  if(typeof callback === 'function') callback(null, this.oauthTokenStore[appid]);
  return this.oauthTokenStore[appid];
};

/**
  * 获取微信 oauth2 授权后通过 code 获取到的信息
  */
exports.getOauthToken = function(appid, callback){
  var token = this.oauthTokenStore ? this.oauthTokenStore[appid] : null;
  if(token){
    if((new Date().getTime()) < token.expireTime) {
      callback(null, token);
    }else {
      this.refreshOauthAccessToken(appid, token.refreshToken, callback);
    }
  }else {
    callback({errMsg: 'refreshToken missing'});
  }
};
...
/**
  * 存储 ticket 函数
  */
exports.saveTicketToken = function(appid, type, ticketToken, callback){
  ...
  if(!this.ticketStore[appid]) this.ticketStore[appid] = {};
  ...
  callback(null, this.ticketStore[appid][type]);
  return this.ticketStore[appid][type];
};

/**
  * 获取 ticket
  */
exports.getTicketToken = function(appid, type, callback){
  if (typeof type === 'function') {
    callback = type;
    type = 'jsapi';
  }
  var ticketToken = (this.ticketToken && this.ticketStore[appid]) ? this.ticketStore[appid][type] : null;
