...
var Trap = module.exports = function(){
  ...
  var self = this;
  var text_handlers = [], click_handlers = {}, regex_media_id = /^[\w\_\-]{40,70}$/,
    ...
    return {
      ...
      image: function(image) {
        ...
        if (typeof image === 'string' && image.match(regex_media_id)) { // image 微信素材id，则直接发送
          ...
          return self.weixin.api.uploadMedia(data[wechatidAttr], image, 'image', function(err, res) {
            /* istanbul ignore if */
      ...
      voice: function(voice) {
        ...
        if (voice.match(regex_media_id)) { // voice 微信素材id，则直接发送
          ...
          return self.weixin.api.uploadMedia(data[wechatidAttr], voice, 'voice', function(err, res) {
            /* istanbul ignore else */
      ...
      video: function(video) {
        ...
        if (video.video.match(regex_media_id)) { // video 微信素材id，则直接发送
          ...
          return self.weixin.api.uploadMedia(data[wechatidAttr], video.video, 'video', function(err, res) {
            /* istanbul ignore else */
      ...
      music: function(music) {
        ...
        if (music.thumb_media.match(regex_media_id)) { // music 微信素材id，则直接发送
          ...
          return self.weixin.api.uploadMedia(data[wechatidAttr], music.thumb_media, 'thumb', function(err, res) {
            /* istanbul ignore else */
  ...
  var getReqBody = function(req, callback) { 
    ...
    /* istanbul ignore if */
    if (!self.getBody || req._body || !regexp.test(mime(req)) || method !== 'post') {
      return callback();
    }
    getBody(req, {
      limit: '100kb',
